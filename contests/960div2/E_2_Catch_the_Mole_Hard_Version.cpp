...
//     no_query = 0;
//     q=0;
//     const int s = 70; //if child has less depth than this - he will eventually rise up to the path chosen
//     vvi g(n+1);
//     rep(i,0,n-1){
//         int a,b; cin>>a>>b;
//         g[a].pb(b);
...
//         if(cur!=1 && g[cur].size()==1) dummy = cur;
//         mxDepth[cur]=1;
//         for(int ch: g[cur]){
//             if(ch==par) continue;
//             self(self, ch, cur);
//             mxDepth[cur] = max(mxDepth[cur], mxDepth[ch]+1);
//         }
//     };
//     dfs(dfs, 1, 0);
//     // deb(mxDepth)
//     vi path;
//     auto findPath = [&](auto& self, int cur, int par)-> void{
//         path.pb(cur);
//         vi good;
//         for(int ch: g[cur]){
//             if(ch==par) continue;
