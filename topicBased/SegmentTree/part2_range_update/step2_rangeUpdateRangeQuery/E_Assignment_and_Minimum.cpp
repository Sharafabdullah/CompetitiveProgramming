...
private:
    ...
    //! apply update should be distributive on the calc -> to make merging easy
    //* Examples: modify with mul and calc sum -> sum every element of the segment after update will simply be the value of parent node * update value
    ...
    ll query(ll l, ll r, ll cur, ll lx, ll rx){
        ...
        ll res = calc_op(query(l, md, L, lx, rx), query(md+1, r, R, lx, rx));
        // apply_update(res, ops[cur]); 
        //* When doing lazy prop - it's insured that seg will contain the correct answer
        // deb(res)
        return res;
    }
