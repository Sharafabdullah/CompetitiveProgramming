...
//! No lazy propogation - not necessary because the update is commutative
struct SegTree{
    #define L 2*cur + 1
    #define R 2*cur + 2
    #define md (l+r)/2
private:
    ...
    void update(int l, int r, int cur, ll val, int lx, int rx){
        ...
        seg[cur] = min(seg[L], seg[R])+ops[cur];
    }
    ...
    ll query(int l, int r, int cur, int lx, int rx){
        ...
        return min(query(l, md, L, lx, rx), query(md+1, r, R, lx,rx)) + ops[cur];
    }
