...
private:
    ...
    pair<ll,ll> merge(pair<ll,ll> x, pair<ll,ll> y){
        pair<ll,ll> ret;
        ret.first = max(x.first, x.second + y.first);
        ret.second = x.second + y.second;
        return ret;
    }
    void build(int l, int r, int node, vi& org){
        if(l==r){ if(l<n) seg[node]={org[l], org[l]}; return;}
    ...
    pair<ll,ll> query(int l, int r, int node, int lq, int rq){
...
public:
    SegTree(vi& arr){
        ...
        seg = vector<pair<ll,ll>>(sz*2,skip);
        build(0, sz-1, 0, arr);
    }
    ...
    ll query(int l, int r){
        return max(query(0, sz-1, 0, l, r).first, 0ll);
    }
#undef L
#undef R
#undef md
