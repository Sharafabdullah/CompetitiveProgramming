...
private:
    ...
    pi query(int l, int r, int node, int val){
        if(l==r){
            return {l, seg[node]};
        }
        if(val <= seg[L]) return query(l, md, L, val);
        else return query(md+1,r, R, val);
    }
...
public:
    ...
    int query(ll val){
        pi ret =  query(0, sz-1, 0, val);
        if(ret.second < val) return -1;
        update(ret.first, ret.second - val);
        return ret.first;
    }
#undef L
#undef R
