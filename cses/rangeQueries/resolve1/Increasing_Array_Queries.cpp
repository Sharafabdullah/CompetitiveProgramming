...
struct Node{
    ll mx = 0, cost = 0, sz = 0, sum = 0;
    Node(){};
    Node(ll mx, ll cost, ll sz, ll sum) : mx(mx), cost(cost), sz(sz), sum(sum){};
};
...
private:
    ...
    T merge(T x, T y){
        Node ret;
        ret.cost = x.cost + max(0ll, x.mx * y.sz - y.sum);
    ...
    void build(int l, int r, int node, vi& org){
        if(l==r){ 
            if(l<org.size()) seg[node]=Node(org[l], 0, 1, org[l]);
            return;
        }
    ...
    void update(int l, int r, int node, int ind, ll val){
        if(l==r){
            seg[node] = Node(val, 0, 1, val);
            return;
        }
