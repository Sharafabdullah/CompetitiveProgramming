...
void solve(){
    ...
    set<int> st;
    map<int, vi> mp;
    rep(i,0,n) {cin>>a[i].first; a[i].second = i; st.insert(a[i].first); mp[a[i].first].pb(i);}
    sort(all(a));
    rep(i,0,n){
        rep(j,i+1,n){
            int rem = target - a[i].first - a[j].first;
            if(rem > 0 && st.count(rem)){
                bool can = 0;
                for(auto org_i: mp[rem]){
                    if(org_i != a[i].second && org_i!=a[j].second) {
                        cout<<org_i + 1<<" ";
                        can = 1;
                        break;
                    }
